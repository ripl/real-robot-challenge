...
class AssertNoStateChanges:
    ...
    def __enter__(self):
        from code.utils import get_body_state
...
def go_to_cleared_position(env, obs, steps=300):
    from code.env.cube_env import ActionType
    from code.const import INIT_JOINT_CONF, CONTRACTED_JOINT_CONF, CLEARED_JOINT_CONF
    counter = 0
    done = False
    initial_position = obs['robot_position']
    while not done and counter < steps:
        ...
        if counter < steps / 2:
            ...
            desired_position = CLEARED_JOINT_CONF

        with action_type_to(ActionType.POSITION, env):
            obs, _, done, _ = env.step(desired_position)
        counter += 1

    return obs


def estimate_object_pose(env, obs, steps):
    '''
    Get the better pose estimation.
    NOTE: This steps the environment
    '''
    def average_euler_angle(angles):
        '''ref: https://en.wikipedia.org/wiki/Mean_of_circular_quantities'''
        return np.arctan2(np.sin(angles).sum(), np.cos(angles).sum())
    from code.const import CUBOID_HALF_SIZE
    from code.env.cube_env import ActionType
    obs = go_to_cleared_position(env, obs)
    done = False
    positions, quats = [], []
    with frameskip_to(1, env):
        with action_type_to(ActionType.POSITION, env):
            zero_action = np.array(obs['robot_position'])
            for _ in range(steps):
                ...
                if done:
                    break
                if (len(positions) > 0
                        and np.allclose(obs['object_position'], positions[-1])
                        and np.allclose(obs['object_orientation'], quats[-1])):
                    continue
                positions.append(obs['object_position'])
                quats.append(obs['object_orientation'])
                t_elapsed = time.time() - t_start
                if t_elapsed < 0.001:
                    time.sleep(0.001 - t_elapsed)
    print(f"N object observations: {len(positions)}")
    avg_pos = np.mean(positions, axis=0)
    est_pos = avg_pos.copy()
    est_pos[2] = CUBOID_HALF_SIZE[0] + 0.001  # observation from simulator
    print('averaged object position:', avg_pos)
    print('amendment to object position:', np.abs(avg_pos - est_pos))
    print('estimated object position:', est_pos)
    yaw = average_euler_angle([
        get_yaw_diff(np.array([0, 0, 0, 1]), quat) for quat in quats
    ])

    target_quats = [
        p.getQuaternionFromEuler((0, pitch, yaw))
        for pitch in [i * np.pi / 2 for i in range(4)]
    ]
    rots = [Rotation.from_quat(q) for q in quats]
    target_rots = [Rotation.from_quat(q) for q in target_quats]
    errors = [
        np.mean([(target_rot.inv() * rot).magnitude() for rot in rots])
        for target_rot in target_rots
    ]
    print('target ori errors: ', errors)
